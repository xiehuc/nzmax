package nz.component
{
	...
	public class Script extends EventDispatcher implements ISaveObject,ILoaderOptimized
	{
		...
		[Event(name = "script_stop", type = "nz.component.Script")]
		[Event(name = "environment_change", type = "nz.component.Script")]
		[Event(name = "script_turnback", type = "nz.component.Script")]
		...
		static public var tagProcessor:Object = new Object();
		/**@private */
		...
		static public function registProcess(name:String,start:Function,end:Function=null,params:Array = null):void
		{
			tagProcessor[name] = [start,end,params];
		}
		...
		public function loadScript(path:String,start:Boolean = false):void
		{
			autostart = start;
			scriptPath = path;
			urlLoader.load(new URLRequest(FileManager.getResolvePath(path)));
		...
		private function progress():void
		{
			while (isrunning) {
				...
				if(tagProcessor[controller] != null){
					var ar:Array=null;
					if(tagProcessor[controller][2]!=null){
						ar = new Array();
						for each(var item:String in tagProcessor[controller][2]){
							if(item == "*")
								ar.push(_cuXML.children());
							else if(item.charAt(0)=='<')
								ar.push(_cuXML.child(item.substr(1,item.length-2))[0]);
							else if(item.charAt(0)=='[')
								ar.push(_cuXML.child(item.substr(1,item.length-2)));
							else
								ar.push(_cuXML.attribute(item));
						}
					}
